...
function FAQItem({
  ...
  return (
    ...
    >
      <button onClick={onToggle} className="w-full flex items-start gap-4 p-5 sm:p-6 text-left group">
        <span
        ...
        <ChevronDown
          size={18}
          className={`shrink-0 mt-0.5 transition-transform duration-300 text-slate-400 ${
            open ? 'rotate-180' : ''
          }`}
          aria-hidden
        />
...
export default function FAQSection({ title = true }: { title?: boolean }) {
      ...
      {title ? (
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-sm" style={{ background: '#0f2d5e' }}>
              <HelpCircle size={18} className="text-white" aria-hidden />
            </div>
            <div>
